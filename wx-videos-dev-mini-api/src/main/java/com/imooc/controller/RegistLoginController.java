...
public class RegistLoginController extends BasicController {
	...
	public IMoocJSONResult login(@RequestBody Users user) throws Exception {
		...
		// 2.判断用户是否存在
		Users userResullt = userService.queryUserForLogin(username, MD5Utils.getMD5Str(user.getPassword()));
		// 3.返回
		if (userResullt != null) {
			userResullt.setPassword("");

			UsersVo usersVo = setUserRedisSessionToken(userResullt);
			return IMoocJSONResult.ok(usersVo);
		} else {
